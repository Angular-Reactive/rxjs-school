...
export default () => {
    ...
    const computeDrawStateFn =  (prevState, event) => {
    ...
    const drawLine1$ = merge(mouseStart$, mouseMove$, mouseEnd$).pipe(
        scan(computeDrawStateFn, {label: 'init'}),
    ...
    drawLine1$.subscribe(data => drawLine(data.origin, data.coords));    
